...
/*   Updated: 2020/03/10 10:06:51 by fballest         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void				ft_setprint5(t_env *env, char c, int i, int z)
{
	...
	j = 0;
	if (env->zero > 0 && env->alig == 0 && env->dot == 0 && (env->type == 'c'
		|| env->type == 's' || env->type == '%' || env->type == 'd'
		|| env->type == 'i' || env->type == 'x' || env->type == 'X'
		|| env->type == 'p' || env->type == 'u'))
		c = '0';
